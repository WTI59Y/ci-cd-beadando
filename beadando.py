...
Builder.load_file("kv-files/snake.kv")
...
class Snake(App):
    ...
    score = kp.NumericProperty(SCORE)
    ...
    def key_handler(self, _, __, ___, key, *____):
        try:
            self.try_change_direction(direction_key[key])
        except KeyError:
            pass
    ...
    def die(self):
        ...
        self.alpha = ALPHA
        # Animation(background_color=(1, 0, 0, 1), duration=MOVESPEED).start(self)
        self.snake.clear()
        self.length = LENGTH
        self.score = SCORE
        self.fruit = self.new_fruit_location
